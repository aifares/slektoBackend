...
async function backfillZones() {
  ...
  while (processed < totalNull) {
    try {
      ...
      console.log(`\n🔍 Processing batch: ${processed + 1} to ${processed + gpsPoints.length} of ${totalNull}`);
      ...
      for (const point of gpsPoints) {
        try {
          ...
          if (zone) {
            ...
            if (updateError) {
              console.error(`❌ Error updating point ${point.id}:`, updateError.message);
              errors++;
            } else {
              updated++;
            }
          ...
          if (processed % 50 === 0) {
            ...
            console.log(`   Progress: ${processed}/${totalNull} (${percentage}%) - Updated: ${updated}, Not in zone: ${notInZone}, Errors: ${errors}`);
          }
        } catch (pointError) {
          console.error(`❌ Error processing point ${point.id}:`, pointError.message);
          errors++;
          processed++;
        }
      }
